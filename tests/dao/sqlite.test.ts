...
test('Get GDPR user data', (t) => {
    ...
    const data = dao.retrieveGDPRData(new URL('https://nightscout-site-one.test'))
...
test('Delete GDPR user data', (t) => {
    ...
    // in the database.
    const leftovers = dao.retrieveGDPRData(url)
