...
Even if you're on Windows, you need access to bash. If you haven't yet, install
WSL which also includes git and bash, using PowerShell in administrator mode: `wsl --install`.

After that, when using Docker Desktop, enable WSL integration at `Settings > Resources > WSL integration`.
...
#### 3. Generate a secret for Iron Session

Nighttune uses [Iron Session](https://github.com/vvo/iron-session) to encrypt your session
cookies, so that they can't be tampered with at the frontend. This key must be generated by you.
There are multiple ways of doing this, of which some are described below. Once you have your
key, store it in your .env file.
...
Head to <https://1password.com/password-generator> and generate your secret.
...
#### Generate a secret key for Iron Session

Follow the instructions at [Step 3 in Using Docker](#3-generate-a-secret-for-iron-session)
